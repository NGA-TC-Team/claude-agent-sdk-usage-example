...
pip install anthropic          # file-upload의 Files API 패턴 전용
...
python file-upload/upload.py              # 패턴 5(Files API)는 anthropic 패키지 필요
...
bun run file-upload/upload.ts             # 패턴 5(Files API)는 bun add @anthropic-ai/sdk 필요
...
- 멀티모달 입력은 `query(prompt=async_gen(), ...)` 형식을 사용한다. `prompt`에 `str` 대신 `AsyncIterable[dict]`를 전달하며, 각 dict는 `{"type": "user", "message": {"role": "user", "content": [블록 배열]}}` 구조다.
...
file-upload/    ← 멀티모달: AsyncIterable prompt로 이미지·문서 content block 전달
...
| `file-upload/sample_test.png` | `upload.py`·`upload.ts` (실행 후 자동 삭제) |

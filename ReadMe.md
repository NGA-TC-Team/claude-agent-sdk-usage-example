...
### `file-upload/` — 멀티모달 파일 전달

텍스트가 아닌 이미지·문서 파일을 Claude에 직접 전달해 분석하는 방법.

- `AsyncIterable` 형식의 `prompt`로 content block 배열 전달 (멀티모달 전용 입력 방식)
- base64 인코딩, 공개 URL 참조, Files API file_id 참조 세 가지 source 타입
- 이미지(JPEG/PNG/GIF/WebP), 문서(PDF/텍스트/마크다운/CSV) 지원 형식 및 크기 제한
- 여러 파일을 한 메시지에 묶어 동시 분석하는 패턴
- Anthropic Files API로 파일 사전 업로드 후 ID 재사용 (별도 `anthropic` 패키지 필요)
...
```
just-chat → keep-history → using-tools → with-streaming → with-mcp → with-workflow → file-upload → application-built-in
```
...
# 7. file-upload  (Files API 패턴은 pip install anthropic / bun add @anthropic-ai/sdk 추가 필요)
python file-upload/upload.py
bun run file-upload/upload.ts

# 8. application-built-in  (내가 만든 앱에 SDK를 심는 패턴)
